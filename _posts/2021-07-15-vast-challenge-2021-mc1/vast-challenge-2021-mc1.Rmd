...
loyalty$popular_location <- paste(loyalty$timestamp,loyalty$location,loyalty$price) # Create a separate column of unique values using concatenated values in the loyalty data
...
route2 <- p5+ ggtitle(label = "Movement Route by Car ID",
                  subtitle = '(Trucks')+theme(
  plot.title = element_text(color = "black",size = 20),
  plot.subtitle = element_text(size = 15))
route2
```
...
In order to link gps and card data, we created a new column to change longitude and latitude into specific location.
```{r}
...
loyaltypriceplot <- ggplot(loyalty, 
                         aes(x = price, 
                         y = reorder(location,price), 
                         fill = "pink"))+
  geom_boxplot() +
  xlab("Price") + ylab("Location") +
  ggtitle("Transactions of each place (Loyalty)") +
  theme(axis.text.x = element_text(face="bold", color="#000092",
                                   size=8, angle=0),
        axis.text.y = element_text(face="bold", color="#000092",
                                   size=8, angle=0))
